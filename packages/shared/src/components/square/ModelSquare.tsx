...
export function ModelSquare({
  ...
  const getModelCoverImage = (index: number, name: string) => {
    ...
    if (lowerName.includes("video") || index === 2) {
      return `${API_BASE}/api/files/model_cg_car.png`;
    }
    if (lowerName.includes("sd") || lowerName.includes("diffusion") || index === 3) {
                      ...
                      >
                        ...
                        >
                          <div className="cover-badge-top">
                            <span className="model-type-badge">{parentName} | {subCat.name}</span>
                          </div>
                          <div className="cover-overlay-glow"></div>
                          <div className="cover-title-bottom">
                            <h4>{subCat.name}</h4>
                          </div>
                        </div>
                        <div className="card-cover-info">
                          <div className="author-row">
                            <div className="author-avatar" style={{ background: "#f5f5f4", color: "#1c1917" }}>#</div>
                            <span className="author-name" style={{ color: "#1c1917", fontWeight: "600" }}>{subCat.name}专区</span>
                          </div>
                          <div className="stats-row">
                            <span className="stat-item"><Star size={11} /> {favCountVal}</span>
                            <span className="stat-item">⚡ {Math.floor((idx + 1) * 314)} 活跃</span>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {catTemplates.length === 0 ? (
                    ...
                    <div className="templates-cover-grid">
                      {catTemplates.map((temp, idx) => {
                        ...
                        return (
                          <div key={temp.id} className="template-cover-card">
                            <div 
                              className="temp-image-header"
                              style={{ backgroundImage: `url(${getModelCoverImage(idx, temp.title)})` }}
                            >
                              <div className="cover-badge-top">
                                <span className="cat-pill">{associatedCat?.name || "精选预设"}</span>
                              </div>
                              <div className="cover-overlay-glow"></div>
                              <button 
                            ...
                            <div className="temp-info-body">
                              <h4>{temp.title}</h4>
                              <div className="prompt-preview-badge">
                                <code>{temp.content}</code>
                              </div>
                              <div className="temp-footer-stats">
                                <span className="stat-item"><Star size={12} /> {Math.floor((idx + 1) * 23)} 收藏</span>
                                <span className="stat-item"><Zap size={12} /> {temp.default_width}×{temp.default_height}</span>
                              </div>
                            </div>
                          </div>
                        );
